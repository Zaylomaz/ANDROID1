...
mixin Scalable {
  ...
  double rescale(Size screenSize, ScalingType scalingType, double value,
      {ScalingSize scalingSize = ScalingSize.small,
      double? minClamp,
      double? maxClamp}) {
    assert(value >= 0, 'value must be greater than or equal to 0');
    assert(minClamp == null || minClamp >= 0,
        'minClamp must be greater than or equal to 0');
    assert(maxClamp == null || maxClamp >= 0,
        'maxClamp must be greater than or equal to 0');
    assert(minClamp == null || maxClamp == null || minClamp <= maxClamp,
        'minClamp must be less than or equal to maxClamp');

    final scalar = scalingSize.scalar;
    var sizeFactor = 1.0;
