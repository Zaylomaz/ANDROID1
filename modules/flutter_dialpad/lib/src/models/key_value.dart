...
class KeyValue {
  const KeyValue(this.value)
      : assert(
          value.length <= 1,
          'value must be a single character or empty',
        );
  final String value;
...
class DigitKey extends KeyValue {
  ...
  /// Returns a [DigitKey] for a given index
  /// using the provided [KeypadIndexedGenerator].
  factory DigitKey.index(int index,
      {KeypadIndexedGenerator generator = const PhoneKeypadGenerator()}) {
    final key = generator.get(index);

    // we will only know what type of key we have at runtime,
    // so we need to assert that it is a DigitKey
    assert(
      key is DigitKey,
      '''generator must return a DigitKey, is your generator configured correctly or check the index value? index: $index, generator key: $key''',
    );
  ...
  factory DigitKey.value(String value) {
    ...
    assert(
        int.tryParse(value) != null, 'value must be a valid numeric character');
    assert(int.parse(value) >= 0 && int.parse(value) <= 9,
        'value must conform to the constraints 0 <= value <= 9');
