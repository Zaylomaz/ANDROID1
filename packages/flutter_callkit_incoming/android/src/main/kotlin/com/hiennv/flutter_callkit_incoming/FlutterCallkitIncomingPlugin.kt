...
class FlutterCallkitIncomingPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    fun getRingtoneSounds() : Array<Ringtones> {
        return Ringtones.values()
    }
    fun playSound(context: Context, sound: Ringtones?) {
        val soundPlayerServiceIntent = Intent(context, CallkitSoundPlayerService::class.java)
        val data = Bundle()
        data.putString(CallkitIncomingBroadcastReceiver.EXTRA_CALLKIT_RINGTONE_PATH, sound?.file)
        data.putBoolean("preview", true)
    ...
    fun stopSound(context: Context) {
        val soundPlayerServiceIntent = Intent(context, CallkitSoundPlayerService::class.java)
        context.stopService(soundPlayerServiceIntent)
    }
...
enum class Ringtones(private val title: String, val file: String) {
    DEFAULT("Default", "ringtone_default"),
    CUSTOM_1("Vertu", "ringtone_custom_1"),
    CUSTOM_2("Vertu Long", "ringtone_custom_2");
