...
import com.rempc.app.eventBus.commands.SipCallStateCommand
...
internal class VideoPreviewHandler : SurfaceHolder.Callback {
    ...
    fun updateVideoPreview(holder: SurfaceHolder) {
        if (SipClient.currentCall != null && SipClient.currentCall?.vidWin != null && SipClient.currentCall?.vidPrev != null) {
            if (videoPreviewActive) {
                ...
                try {
                    SipClient.currentCall?.vidPrev?.start(vidPrevParam)
                } catch (e: Exception) {
                    println(e)
                }
            } else {
                try {
                    SipClient.currentCall?.vidPrev?.stop()
                } catch (e: Exception) {
                    println(e)
                }
            }
...
class CallActivity : Activity(), Handler.Callback, SurfaceHolder.Callback {
...
//        val buttonShowPreview = findViewById<View>(R.id.buttonShowPreview) as Button
        if (SipClient.currentCall == null ||
                SipClient.currentCall?.vidWin == null
        ) {
...
//        surfacePreview.holder.addCallback(previewHandler)
        handler_ = handler
        SipClient.currentCall?.let {
            try {
                lastCallInfo = SipClient.lastCallInfo
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        ...
        SipClient.account?.let { acc ->
            try {
                val cfg = acc.cfg
                val cap_dev = cfg.videoConfig.defaultCaptureDevice
                SipApp.endpoint?.vidDevManager()?.setCaptureOrient(
                        cap_dev, orient,
                        true
                )
            } catch (e: Exception) {
    ...
    private fun updateVideoWindow(show: Boolean) {
        if (SipClient.currentCall != null && SipClient.currentCall?.vidWin != null && SipClient.currentCall?.vidPrev != null) {
            ...
            try {
                SipClient.currentCall?.vidWin?.setWindow(vidWH)
            } catch (e: Exception) {
                println(e)
            }
        }
...
//        EventBus.getDefault().post(AnswerCallCommand())
//        val prm = CallOpParam()
//        prm.statusCode = pjsip_status_code.PJSIP_SC_OK
//        try {
//            SipClient.currentCall?.answer(prm)
//        } catch (e: Exception) {
//            println(e)
//        }
...
//                if (previewHandler.videoPreviewActive) "hide" else "show"
//    }
...
//        buttonShowPreview.visibility = View.VISIBLE
    ...
    override fun handleMessage(m: Message): Boolean {
        if (m.what == SipClient.MSG_TYPE.CALL_STATE) {
            lastCallInfo = m.obj as CallInfo
            updateCallState(lastCallInfo)
        } else if (m.what == SipClient.MSG_TYPE.CALL_MEDIA_STATE) {
            if (SipClient.currentCall?.vidWin != null) {
